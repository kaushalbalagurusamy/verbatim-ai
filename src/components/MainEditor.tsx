...
export function MainEditor() {
  const [tabs, setTabs] = useState([
    { id: 1, title: 'New Notepad', active: true, modified: false }
  ]);
  ...
  return (
    <div className="flex flex-col h-screen">
      {/* Title Bar */}
      <div className="h-8 bg-[#3c3c3c] flex items-center justify-between px-4 border-b border-[#3c3c3c]">
        <div className="flex items-center gap-2">
          <span className="text-xs text-[#cccccc]">New Notepad — logos-ai</span>
        </div>
        <div className="flex items-center gap-1">
          ...
          <button className="p-1 hover:bg-[#4c4c4c] rounded">
            <MoreHorizontal className="w-3 h-3 text-[#cccccc]" />
          </button>
        </div>
      </div>
      ...
      <div className="h-9 bg-[#2d2d30] border-b border-[#3c3c3c] flex items-center">
        {tabs.map((tab) => (
          <div
            key={tab.id}
            className={`flex items-center gap-2 px-3 h-full border-r border-[#3c3c3c] min-w-0 max-w-xs ${
      ...
      <div className="flex-1 bg-[#1e1e1e] p-6">
        <div className="max-w-4xl">
          ...
          <div className="min-h-96">
            <textarea
              className="w-full h-full bg-transparent text-[#cccccc] border-none outline-none resize-none text-sm leading-relaxed"
